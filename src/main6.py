...
def summarize_text(text: str, tokenizer, model, subtitle_file):
    inputs = tokenizer(text, return_tensors="pt")
    outputs = model.generate(**inputs, max_new_tokens=100)
    # inputs = tokenizer.encode(text, return_tensors="pt")
    # outputs = model.generate(inputs, max_length=150)
    summ = tokenizer.decode(outputs[0], skip_special_tokens=True)
    with open(f'/home/morlot/code/audiotxt/output/{subtitle_file}', "w") as text_file:
...
async def summarize_text_api(request: Request, filename: str = "subtitles",):
    ...
    summary = summarize_text(text, tokenizer, model, subtitle_file)
